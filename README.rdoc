...
And play around with I18n:
  ...
  puts Category.first.name_en
  puts Category.first.name_es
...
== Please help
I need help refactoring and improving the methods in active_record.rb so that the methods are defined in a more decent way.

== Todo's

* Support class namespaces
* I need help refactoring and improving the methods in active_record.rb so that the methods are defined in a more decent way, e.g. defining them using <code>define_method</code>. Please help!
* Cache translations on application level so they won't be loaded for each translated record
